...
pub const Shape = union(enum) {
    ...
    pub fn intersect(self: *const Shape, probe: Probe, trafo: Trafo, current_time_start: u64, isec: *Intersection) bool {
        return switch (self.*) {
            ...
            .PointMotionCloud => |c| c.intersect(probe, trafo, current_time_start, isec),
    ...
    pub fn fragment(self: *const Shape, probe: Probe, current_time_start: u64, frag: *Fragment) void {
        switch (self.*) {
            ...
            .PointMotionCloud => |c| c.fragment(probe, current_time_start, frag),
    ...
    pub fn intersectP(self: *const Shape, probe: Probe, trafo: Trafo, current_time_start: u64) bool {
        return switch (self.*) {
            .Cube => Cube.intersectP(probe.ray, trafo),
            .CurveMesh => |m| m.intersectP(probe.ray, trafo),
            .Disk => Disk.intersectP(probe.ray, trafo),
            .PointMotionCloud => |c| c.intersectP(probe, trafo, current_time_start),
    ...
    pub fn surfaceDifferentials(self: *const Shape, primitive: u32, trafo: Trafo) DifferentialSurface {
        return switch (self.*) {

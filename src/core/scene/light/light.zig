...
pub const Light = struct {
    ...
    pub fn evaluateTo(self: Light, p: Vec4f, sample: *const SampleTo, filter: ?Filter, sampler: *Sampler, scene: *const Scene) Vec4f {
        const material = scene.propMaterial(self.prop, self.part);

        return material.evaluateRadiance(p, sample.wi, sample.n, sample.uvw, sample.trafo, self.extent, filter, sampler, scene);
    }

    pub fn evaluateFrom(self: Light, p: Vec4f, sample: *const SampleFrom, filter: ?Filter, sampler: *Sampler, scene: *const Scene) Vec4f {
        const material = scene.propMaterial(self.prop, self.part);

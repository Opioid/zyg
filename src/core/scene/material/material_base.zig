...
pub const Base = struct {
    ...
    pub inline fn border(self: Base, wi: Vec4f, n: Vec4f) f32 {
    ...
    pub inline fn similarityRelationScale(self: Base, depth: u32) f32 {
        const gs = self.vanDeHulstAnisotropy(depth);
        return vanDeHulst(self.volumetric_anisotropy, gs);
    }

    pub inline fn vanDeHulstAnisotropy(self: Base, depth: u32) f32 {
        if (depth < SR_low) {

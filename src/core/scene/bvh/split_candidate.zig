...
pub const SplitCandidate = struct {
    ...
    pub fn evaluate(self: *Self, references: []const Reference, aabb_surface_area: f32) void {
        ...
        if (empty_side) {
            self.cost = 2.0 + @floatFromInt(f32, references.len);
        } else {
            const weight_0 = @floatFromInt(f32, num_sides[0]) * aabbs[0].surfaceArea();
            const weight_1 = @floatFromInt(f32, num_sides[1]) * aabbs[1].surfaceArea();

            const duplication_penalty = 0.125 * @floatFromInt(f32, num_sides[0] + num_sides[1] - references.len);

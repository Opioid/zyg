...
pub const Noise = struct {
    ...
    fn perlin3D_1(p: Vec4f) f32 {
        ...
        const result0 = math.bilinear(f32, c0, uvw[0], uvw[1]);

        const c1 = gradient3v(hash3v(P0, P1, @splat(P[2] + 1)), fp0, fp1, @splat(fp[2] - 1.0));

        const result1 = math.bilinear(f32, c1, uvw[0], uvw[1]);

        return gradient_scale3D(math.lerp(result0, result1, uvw[2]));
    }
    ...
    fn bjfinal(a_in: anytype, b_in: @TypeOf(a_in), c_in: @TypeOf(a_in)) @TypeOf(a_in) {
        var a = a_in;
        var b = b_in;
        var c = c_in;

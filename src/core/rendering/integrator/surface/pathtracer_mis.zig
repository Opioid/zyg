...
pub const PathtracerMIS = struct {
    ...
    fn connectLight(
        ...
        if (frag.hit()) {
            result.emission = context.evaluateRadiance(vertex, frag, split_threshold, sampler);
        }
        ...
        if (ro.RayMaxT == vertex.probe.ray.max_t) {
            for (context.scene.infinite_props.items) |prop| {
                ...
                const local_energy = context.evaluateRadiance(vertex, &light_frag, split_threshold, sampler);

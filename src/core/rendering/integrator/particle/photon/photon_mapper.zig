...
pub const Mapper = struct {
    ...
    fn tracePhoton(self: *Self, bounds: AABB, frame: u32, max_photons: u32, finite_world: bool, worker: *Worker) Result {
        ...
        while (i < Max_iterations) : (i += 1) {
            ...
            var radiance = light.evaluateFrom(isec.geo.p, light_sample, filter, &self.sampler, worker.scene) / @splat(4, light_sample.pdf());
            radiance *= throughput;
            ...
            while (ray.depth < self.settings.max_bounces) {
                ...
                if (sample_result.class.transmission) {
                    const ior = worker.interfaceChangeIor(sample_result.wi, isec, filter, &self.sampler);

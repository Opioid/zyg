...
fn TSensor(comptime ImportanceSampling: bool) type {
    return struct {
        ...
        const N = 30;
        const Func = math.ifunc.InterpolatedFunction1DN(N);
        ...
        pub fn cameraSample(self: *const Self, pixel: Vec2i, sampler: *Sampler) Sample {
            ...
            if (ImportanceSampling) {
                ...
                return .{
                    .pixel = pixel,
                    .filter_uv = pixel_uv - @as(Vec2f, @splat(0.5)),
                    .lens_uv = .{ s4[2], s4[3] },
                    .time = s1,
                };
            }
        ...
        pub fn addSample(
            ...
            if (ImportanceSampling) {
                ...
                const filter_uv = sample.filter_uv;
                const ox = filter_uv[0];
                const oy = filter_uv[1];
                ...
                if (0 == self.filter_radius_int) {
                        ...
                        while (i < len) : (i += 1) {
                            ...
                            if (aov.activeClass(class)) {
                                ...
                                if (.Depth == class) {
                                    ...
                                    layer.aov.overwritePixel(id, i, avalue[0], 1.0);
                                } else {
                    ...
                    layer.buffer.addPixel(id, composed, 1.0);
                    ...
                    // 1. row
                    self.add(layer_id, .{ x - 1, y - 1 }, wx0 * wy0, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y - 1 }, wx1 * wy0, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y - 1 }, wx2 * wy0, composed, bounds, isolated);

                    // 2. row
                    self.add(layer_id, .{ x - 1, y }, wx0 * wy1, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y }, wx1 * wy1, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y }, wx2 * wy1, composed, bounds, isolated);

                    // 3. row
                    self.add(layer_id, .{ x - 1, y + 1 }, wx0 * wy2, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y + 1 }, wx1 * wy2, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y + 1 }, wx2 * wy2, composed, bounds, isolated);
                    ...
                    if (aov.active()) {
                        ...
                        while (i < len) : (i += 1) {
                            ...
                            if (aov.activeClass(class)) {
                                ...
                                if (.Depth == class) {
                                    self.lessAov(layer_id, .{ x, y }, i, avalue[0], bounds);
                                } else if (.MaterialId == class) {
                                    self.overwriteAov(layer_id, .{ x, y }, i, wx2 * wy2, avalue[0], bounds);
                                } else {
                                    // 1. row
                                    self.addAov(layer_id, .{ x - 1, y - 1 }, i, wx0 * wy0, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y - 1 }, i, wx1 * wy0, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y - 1 }, i, wx2 * wy0, avalue, bounds, isolated);

                                    // 2. row
                                    self.addAov(layer_id, .{ x - 1, y }, i, wx0 * wy1, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y }, i, wx1 * wy1, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y }, i, wx2 * wy1, avalue, bounds, isolated);

                                    // 3. row
                                    self.addAov(layer_id, .{ x - 1, y + 1 }, i, wx0 * wy2, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y + 1 }, i, wx1 * wy2, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y + 1 }, i, wx2 * wy2, avalue, bounds, isolated);
                                }
                            }
                        }
                    }
                    ...
                    // 1. row
                    self.add(layer_id, .{ x - 2, y - 2 }, wx0 * wy0, composed, bounds, isolated);
                    self.add(layer_id, .{ x - 1, y - 2 }, wx1 * wy0, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y - 2 }, wx2 * wy0, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y - 2 }, wx3 * wy0, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 2, y - 2 }, wx4 * wy0, composed, bounds, isolated);

                    // 2. row
                    self.add(layer_id, .{ x - 2, y - 1 }, wx0 * wy1, composed, bounds, isolated);
                    self.add(layer_id, .{ x - 1, y - 1 }, wx1 * wy1, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y - 1 }, wx2 * wy1, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y - 1 }, wx3 * wy1, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 2, y - 1 }, wx4 * wy1, composed, bounds, isolated);

                    // 3. row
                    self.add(layer_id, .{ x - 2, y }, wx0 * wy2, composed, bounds, isolated);
                    self.add(layer_id, .{ x - 1, y }, wx1 * wy2, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y }, wx2 * wy2, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y }, wx3 * wy2, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 2, y }, wx4 * wy2, composed, bounds, isolated);

                    // 4. row
                    self.add(layer_id, .{ x - 2, y + 1 }, wx0 * wy3, composed, bounds, isolated);
                    self.add(layer_id, .{ x - 1, y + 1 }, wx1 * wy3, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y + 1 }, wx2 * wy3, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y + 1 }, wx3 * wy3, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 2, y + 1 }, wx4 * wy3, composed, bounds, isolated);

                    // 5. row
                    self.add(layer_id, .{ x - 2, y + 2 }, wx0 * wy4, composed, bounds, isolated);
                    self.add(layer_id, .{ x - 1, y + 2 }, wx1 * wy4, composed, bounds, isolated);
                    self.add(layer_id, .{ x, y + 2 }, wx2 * wy4, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 1, y + 2 }, wx3 * wy4, composed, bounds, isolated);
                    self.add(layer_id, .{ x + 2, y + 2 }, wx4 * wy4, composed, bounds, isolated);
                        ...
                        while (i < len) : (i += 1) {
                            ...
                            if (aov.activeClass(class)) {
                                ...
                                if (.Depth == class) {
                                    ...
                                    // 1. row
                                    self.addAov(layer_id, .{ x - 2, y - 2 }, i, wx0 * wy0, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x - 1, y - 2 }, i, wx1 * wy0, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y - 2 }, i, wx2 * wy0, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y - 2 }, i, wx3 * wy0, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 2, y - 2 }, i, wx4 * wy0, avalue, bounds, isolated);

                                    // 2. row
                                    self.addAov(layer_id, .{ x - 2, y - 1 }, i, wx0 * wy1, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x - 1, y - 1 }, i, wx1 * wy1, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y - 1 }, i, wx2 * wy1, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y - 1 }, i, wx3 * wy1, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 2, y - 1 }, i, wx4 * wy1, avalue, bounds, isolated);

                                    // 3. row
                                    self.addAov(layer_id, .{ x - 2, y }, i, wx0 * wy2, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x - 1, y }, i, wx1 * wy2, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y }, i, wx2 * wy2, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y }, i, wx3 * wy2, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 2, y }, i, wx4 * wy2, avalue, bounds, isolated);

                                    // 4. row
                                    self.addAov(layer_id, .{ x - 2, y + 1 }, i, wx0 * wy3, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x - 1, y + 1 }, i, wx1 * wy3, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y + 1 }, i, wx2 * wy3, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y + 1 }, i, wx3 * wy3, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 2, y + 1 }, i, wx4 * wy3, avalue, bounds, isolated);

                                    // 5. row
                                    self.addAov(layer_id, .{ x - 2, y + 2 }, i, wx0 * wy4, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x - 1, y + 2 }, i, wx1 * wy4, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x, y + 2 }, i, wx2 * wy4, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 1, y + 2 }, i, wx3 * wy4, avalue, bounds, isolated);
                                    self.addAov(layer_id, .{ x + 2, y + 2 }, i, wx4 * wy4, avalue, bounds, isolated);
                                }
                            }
                        }
                    }
        ...
        pub fn resolve(self: *const Self, layer: u32, target: [*]Pack4f, num_pixels: u32, threads: *Threads) void {
        ...
        pub fn resolveTonemap(self: *const Self, layer: u32, target: [*]Pack4f, num_pixels: u32, threads: *Threads) void {
            var context = ResolveContext{ .sensor = self, .target = target, .layer = layer, .aov = .Albedo };
            _ = threads.runRange(&context, ResolveContext.resolveTonemap, 0, num_pixels, @sizeOf(Vec4f));
        }

        pub fn resolveAccumulateTonemap(self: *const Self, layer: u32, target: [*]Pack4f, num_pixels: u32, threads: *Threads) void {
            var context = ResolveContext{ .sensor = self, .target = target, .layer = layer, .aov = .Albedo };
            _ = threads.runRange(&context, ResolveContext.resolveAccumulateTonemap, 0, num_pixels, @sizeOf(Vec4f));
        }

        pub fn resolveAov(self: *const Self, layer: u32, class: AovValue.Class, target: [*]Pack4f, num_pixels: u32, threads: *Threads) void {
        ...
        fn add(self: *Self, layer: u32, pixel: Vec2i, weight: f32, color: Vec4f, bounds: Vec4i, isolated: Vec4i) void {
                ...
                @as(u32, @bitCast(pixel[1] - bounds[1])) <= @as(u32, @bitCast(bounds[3])))
            {
                ...
                {
                    _ = self.layers[layer].buffer.addPixel(i, color, weight);
                } else {
                    _ = self.layers[layer].buffer.addPixelAtomic(i, color, weight);
                }
            }
        }
